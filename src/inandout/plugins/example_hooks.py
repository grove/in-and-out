...
def get_hooks() -> dict[str, ConnectorHooks]:
    ...
    This function is the entry point factory called by the plugin discovery
    system.

    Returns
    -------
    dict[str, ConnectorHooks]
        Keys are connector names (must match the ``name`` field in the
        connector YAML).  Values are ``ConnectorHooks`` instances with any
        combination of transform/filter/enrich hooks set.
    """
    return {
