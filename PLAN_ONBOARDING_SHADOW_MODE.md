...
| **Re-merge deltas hit systems 1 and 2 immediately** | Because system 3 is in the production pipeline from the start, cluster re-merges affect systems 1 and 2's writeback immediately. If the re-merge is wrong, it's already been executed. | High — this is the tradeoff for avoiding blue/green's full duplication. Can be mitigated by combining shadow mode with delta gating for systems 1 and 2 during the initial resolution. |
...
| **Storage overhead** | Minimal | 2× full pipeline | Shadow table only |
| **Review window** | Fixed (gate → review → ungate) | Unlimited (parallel) | Continuous (converges over time) |
| **Systems 1 & 2 downtime** | Optional (Mode A: none; Mode B: paused) | None | None (re-merge deltas flow immediately) |
| **First writeback blast radius** | Full golden record delta | Full golden record delta (after cutover) | Only genuine gaps (filtered by shadow) |
...
| **New in-and-out features** | Control table commands | Blue pipeline generator | Shadow observer daemon, comparison logic |
| **Automation potential** | High (threshold-gated) | High (state machine) | Highest (continuous convergence) |
| **Implementation complexity** | Low | Medium | High |
| **API cost during onboarding** | None (read-only ingestion) | Doubled (dual ingestion) or none (DB replication) | Medium (comparison fetches) |
...
4. **Shadow for systems 1 and 2:** Could shadow mode also be used for systems 1 and 2 during the re-merge window? Instead of immediately writing re-merge deltas to systems 1 and 2, shadow them first. This provides the same safety as Mode B (conservative gating) but with convergence visibility. The cost is additional API calls to systems 1 and 2.
